...
`ok` 表示本次硬件核心探测是否成功：`OK` 和 `WARN` 均为 `ok=true`，只有 `FAIL` 为 `ok=false`。因此不能直接用当前 `fail(..., severity=WARN)` 或 `merge()` 生成 warning success；应显式构造结果或增加硬件域 helper，保持原有公共 API 不被本阶段顺手改写。

`--all` 返回一个顶层 `CheckResult`，`data.results` 按配置顺序保留每台服务器结果，并额外给出 `passed_servers`、`failed_servers` 和计数。只要一台服务器失败，顶层 `ok=false`、退出码 1；只有 warning 时顶层 `ok=true/severity=warn`、退出码 0；其他 future 完成后仍要输出。
