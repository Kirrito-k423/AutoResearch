...
def capture_repo_provenance(
    ...
    *,
    ...
    if allow_commit_push:
        ...
        if branch:
            code, _stdout, _stderr = runner(["git", "push", "-u", "origin", branch], root)
            if code == 0 and origin_url:
                pushed_url = _branch_url(origin_url, branch)

    commit_sha = _git(runner, root, "rev-parse", "HEAD")
    fork_url = None
    source_url = upstream_url or origin_url
    if source_url and fork_owner:
        fork_url = _fork_url(source_url, fork_owner)
