...
- **参数名**：`actor_rollout_ref.actor.megatron.param_offload`
- **分类**：效率
- **中文解释**：把参数、梯度或优化器状态卸载到 CPU/主机侧，降低 HBM 压力但可能拖慢训练。
- **常见值**：False、True
- **来源环境变量**：ALL_OFFLOAD、OFFLOAD
...
- `examples/dppo_trainer/run_qwen3_30b_a3b_megatron.sh:90` actor_rollout_ref.actor.megatron.param_offload=True
- `examples/gspo_trainer/run_qwen3_30b_a3b_megatron.sh:81` actor_rollout_ref.actor.megatron.param_offload=True
- `examples/mtp_trainer/run_mimo_7b_mtp_fully_async_megatron_multinode.sh:102` actor_rollout_ref.actor.megatron.param_offload=False \
- `examples/mtp_trainer/run_mimo_7b_mtp_megatron.sh:89` actor_rollout_ref.actor.megatron.param_offload=True
- `examples/mtp_trainer/run_mimo_7b_mtp_rl_vllm_sgl_megatron.sh:107` actor_rollout_ref.actor.megatron.param_offload=True
