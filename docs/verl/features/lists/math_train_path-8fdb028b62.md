...
- **参数名**：`math_train_path`
- **分类**：配置
- **中文解释**：MATH 训练集 parquet 路径；示例把它与 `gsm8k_train_path` 组装成 `train_files`，再传给 `data.train_files` 作为 RL 训练数据来源。
- **常见值**：$HOME/data/math/train.parquet
- **来源环境变量**：math_train_path
...
- `examples/grpo_trainer/run_qwen2-7b_math_megatron_fsdp.sh:22` math_train_path=${math_train_path:-$HOME/data/math/train.parquet}
