...
- **参数名**：`ROLLOUT_IS`
- **分类**：效率
- **中文解释**：文档说明：Rollout Correction 中的 importance sampling（IS）权重聚合级别；`token` 表示逐 token 权重，`sequence` 表示整段回复共享序列级权重，`null` 则关闭 IS。
- **常见值**：sequence
- **来源环境变量**：ROLLOUT_IS
...
- `examples/grpo_trainer/run_qwen3_vl_30b_a3b_fsdp.sh:39` ROLLOUT_IS=${ROLLOUT_IS:-sequence}
