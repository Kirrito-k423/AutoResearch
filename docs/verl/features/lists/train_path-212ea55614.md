...
- **参数名**：`train_path`
- **分类**：配置
- **中文解释**：通用/Geo3K 训练集 parquet 路径；示例直接传给 `data.train_files`，作为 RL 训练数据来源。
- **常见值**：$HOME/data/geo3k/train.parquet
- **来源环境变量**：train_path
...
- `examples/grpo_trainer/run_qwen3_5_35b_megatron.sh:85` train_path=${train_path:-$HOME/data/geo3k/train.parquet}
