...
- **参数名**：`data.train_batch_size`
- **分类**：效率
- **中文解释**：控制训练全局 batch，影响样本吞吐、梯度稳定性和单步耗时。
- **常见值**：$train_batch_size、0、1024、128、16、192、256、3、32、4、512、64
- **来源环境变量**：ROLLOUT_BATCH_SIZE、TRAIN_BATCH_SIZE、train_batch_size
...
- `examples/dppo_trainer/run_qwen3_30b_a3b_megatron.sh:62` data.train_batch_size=${train_batch_size}
- `examples/reinforce_plus_plus_trainer/run_qwen3_8b_fsdp.sh:49` data.train_batch_size=${train_batch_size}
- `examples/gmpo_trainer/run_qwen3_8b_fsdp.sh:50` data.train_batch_size=${train_batch_size}
- `examples/gspo_trainer/run_qwen3_8b_fsdp.sh:87` data.train_batch_size=${train_batch_size}
- `examples/gspo_trainer/run_qwen3_30b_a3b_megatron.sh:53` data.train_batch_size=${train_batch_size}
